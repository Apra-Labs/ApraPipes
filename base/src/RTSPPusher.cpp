...
class RTSPPusher::Detail {
  ...
  bool init_stream_params() {
    ...
    H264ParserUtils::parse_sps(
        ((const char *)sps_pps.data()) + 5,
        sps_pps.size() > 5 ? sps_pps.size() - 5 : sps_pps.size(), &p);
    width = p.width;
    height = p.height;
    bitrate = 2000000;
    fps_den = 30;
...
public:
  ...
  bool write_precoded_video_frame(boost::shared_ptr<Frame> &f) {
    ...
    if (ret < 0) {
      bRC = false;
      // av_log(NULL, AV_LOG_ERROR, "Error while writing video frame.ret =
      // %d\n", ret);
  ...
  Detail(RTSPPusherProps props)
      : mURL(props.URL), mTitle(props.title), isTCP(props.isTCP),
  ...
  bool init() {
    ...
    if (!outContext) {
      // av_log(NULL, AV_LOG_FATAL, "Could not allocate an output context for
      // '%s'.\n", url);
  ...
  bool write_header(int num, int den) {
    ...
    if (video_st) {
      ...
      LOG_FATAL << "Add video stream for the codec : ^"
  ...
  void calculateDuration(int den = 0) {
    ...
    duration =
        av_rescale_q_rnd(1, in_time_base, timeBase,
...
RTSPPusher::RTSPPusher(RTSPPusherProps props)
    : Module(SINK, "RTSPPusher", props) {
...
bool RTSPPusher::term() {
  ...
  if (mDetail->connectionStatus == WRITE_FAILED ||
...
bool RTSPPusher::shouldTriggerSOS() {
  std::string err = mDetail->connectionStatus == CONNECTION_FAILED
                        ? "connection failed"
                        : "stream ended";
