...
public:
  DTSFixedRateStrategy()
      : DTSCalcStrategy(DTSCalcStrategy::DTSCalcStrategyType::FIXED_RATE) {}
  int64_t getDTS(uint64_t &frameTS, uint64_t lastFrameTS,
                 uint16_t fps) override {
...
public:
  ...
  void setImageMetadata(framemetadata_sp &metadata) {
    ...
    if (mFrameType == FrameMetadata::FrameType::ENCODED_IMAGE) {
      auto encodedImageMetadata =
          FrameMetadataFactory::downcast<EncodedImageMetadata>(metadata);
      ...
      auto h264ImageMetadata =
          FrameMetadataFactory::downcast<H264Metadata>(metadata);
  ...
  void initNewMp4File(std::string &filename) {
    ...
    if (mMetadataEnabled) {
      ...
      auto ret = mp4_mux_track_set_metadata_mime_type(
          mux, metatrack, content_encoding.c_str(), mime_format.c_str());
      if (ret != 0) {
...
public:
  ...
  bool write(frame_container &frames);
  void modifyFrameOnNewSPSPPS(short naluType, frame_sp frame,
                              uint8_t *&spsPpsdata, size_t &spsPpsSize,
                              uint8_t *&frameData, size_t &frameSize);
  ...
  bool set_video_decoder_config() {
    ...
    vdc.avc.sps =
        reinterpret_cast<uint8_t *>(const_cast<void *>(spsBuffer.data()));
    vdc.avc.pps =
        reinterpret_cast<uint8_t *>(const_cast<void *>(ppsBuffer.data()));
...
bool DetailJpeg::write(frame_container &frames) {
  auto inJpegImageFrame =
      Module::getFrameByType(frames, FrameMetadata::FrameType::ENCODED_IMAGE);
  auto inMp4MetaFrame = Module::getFrameByType(
      frames, FrameMetadata::FrameType::MP4_VIDEO_METADATA);
  if (!inJpegImageFrame) {
  ...
  mWriterSinkUtils.getFilenameForNextFrame(
      _nextFrameFileName, inJpegImageFrame->timestamp, mProps->baseFolder,
      mProps->chunkTime, mProps->syncTimeInSecs, syncFlag, mFrameType,
      naluType);
  ...
  if (!lastFrameTS) {
    ...
    diffInMsecs =
        mDTSCalc->getDTS(inJpegImageFrame->timestamp, lastFrameTS, mProps->fps);
  }
  lastFrameTS = inJpegImageFrame->timestamp;
  mux_sample.dts =
      mux_sample.dts +
      static_cast<int64_t>((params.timescale / 1000) * diffInMsecs);
...
void DetailH264::modifyFrameOnNewSPSPPS(short naluType,
                                        frame_sp inH264ImageFrame,
                                        uint8_t *&spsPpsBuffer,
                                        size_t &spsPpsSize, uint8_t *&frameData,
                                        size_t &frameSize) {
  ...
  // add the size of sps to the 4th byte of sps's nalu seprator (00 00 00
  // SpsSize 67)
  ...
  // add the size of sps to the 4th byte of pps's nalu seprator (00 00 00
  // PpsSize 68)
  memcpy(spsPpsBuffer, nalu, 3);
  spsPpsBuffer += 3;
...
bool DetailH264::write(frame_container &frames) {
  auto inH264ImageFrame =
      Module::getFrameByType(frames, FrameMetadata::FrameType::H264_DATA);
  auto inMp4MetaFrame = Module::getFrameByType(
      frames, FrameMetadata::FrameType::MP4_VIDEO_METADATA);
  if (!inH264ImageFrame) {
    LOG_ERROR << "Image Frame is empty. Unable to write.";
  ...
  auto mFrameBuffer =
      const_buffer(inH264ImageFrame->data(), inH264ImageFrame->size());
  ...
  if (mNextFrameFileName != _nextFrameFileName) {
    ...
    if (naluType == H264Utils::H264_NAL_TYPE_IDR_SLICE ||
      ...
      // new video
      modifyFrameOnNewSPSPPS(naluType, inH264ImageFrame, spsPpsBuffer,
                             spsPpsSize, frameData, frameSize);
    ...
    // new sps pps
    modifyFrameOnNewSPSPPS(naluType, inH264ImageFrame, spsPpsBuffer, spsPpsSize,
                           frameData, frameSize);
    prepend_buffer.buffer = spsPpsBuffer;
    prepend_buffer.len = spsPpsSize;
    mux_sample.buffer = frameData;
    ...
    // assign size of the frame to the NALU seperator for playability in default
    // players
  ...
  if (naluType == H264Utils::H264_NAL_TYPE::H264_NAL_TYPE_IDR_SLICE ||
      naluType == H264Utils::H264_NAL_TYPE::H264_NAL_TYPE_SEQ_PARAM) {
    isKeyFrame = true;
  }
  ...
  if (!lastFrameTS) {
    ...
    diffInMsecs =
        mDTSCalc->getDTS(inH264ImageFrame->timestamp, lastFrameTS, mProps->fps);
  }
  ...
  mp4_mux_track_add_sample_with_prepend_buffer(mux, videotrack, &prepend_buffer,
                                               &mux_sample);

  if (metatrack != -1 && mMetadataEnabled) {
    // null check must happen here to ensure 0 sized entries in mp4 metadata
    // track's stsz table this will ensure equal entries in metadata and video
    // tracks
...
bool Mp4WriterSink::init() {
  ...
  for (auto const &element : inputPinIdMetadataMap) {
    ...
    else if (mFrameType == FrameMetadata::FrameType::MP4_VIDEO_METADATA &&
             mProp.enableMetadata) {
...
bool Mp4WriterSink::validateInputOutputPins() {
  // if (getNumberOfInputsByType(FrameMetadata::H264_DATA) != 1 &&
  // getNumberOfInputsByType(FrameMetadata::ENCODED_IMAGE) != 1)
  // {
  // 	LOG_ERROR << "<" << getId() << ">::validateInputOutputPins expected 1
  // pin of ENCODED_IMAGE. Actual<" << getNumberOfInputPins() << ">"; 	return
  // false;
  // }
  return true;
}
...
bool Mp4WriterSink::validateInputPins() {
  ...
  // be 2. Actual<" << getNumberOfInputPins() << ">"; 	return false;
  // }
  ...
  for (auto const &element : inputPinIdMetadataMap) {
    auto &metadata = element.second;
    auto mFrameType = metadata->getFrameType();
    if (mFrameType != FrameMetadata::ENCODED_IMAGE &&
        mFrameType != FrameMetadata::MP4_VIDEO_METADATA &&
        mFrameType != FrameMetadata::H264_DATA) {
      LOG_ERROR << "<" << getId()
                << ">::validateInputPins input frameType is expected to be "
                   "ENCODED_IMAGE or MP4_VIDEO_METADATA. Actual<"
                << mFrameType << ">";
      return false;
    }
    ...
    if (memType != FrameMetadata::MemType::HOST) {
      LOG_ERROR << "<" << getId()
                << ">::validateInputPins input memType is expected to be HOST. "
                   "Actual<"
                << memType << ">";
      return false;
    }
...
bool Mp4WriterSink::enableMp4Metadata(framemetadata_sp &inputMetadata) {
  auto mp4VideoMetadata =
      FrameMetadataFactory::downcast<Mp4VideoMetadata>(inputMetadata);
  std::string formatVersion = mp4VideoMetadata->getVersion();
  if (formatVersion.empty()) {
    LOG_ERROR << "Serialization Format Information missing from the metadata. "
                 "Metadata writing will be disabled";
    return false;
  }
...
bool Mp4WriterSink::processSOS(frame_sp &frame) {
  ...
  if (mFrameType == FrameMetadata::FrameType::ENCODED_IMAGE ||
      mFrameType == FrameMetadata::FrameType::H264_DATA) {
  ...
  if (mFrameType == FrameMetadata::FrameType::MP4_VIDEO_METADATA) {
    ...
    std::string formatVersion = mp4VideoMetadata->getVersion();
    if (formatVersion.empty()) {
      LOG_ERROR << "Serialization Format Information missing from the "
                   "metadata. Metadata writing will be disabled";
      return true;
    }
    mDetail->enableMetadata(formatVersion);
  }
...
bool Mp4WriterSink::shouldTriggerSOS() { return mDetail->shouldTriggerSOS(); }
...
bool Mp4WriterSink::processEOS(string &pinId) { return true; }
