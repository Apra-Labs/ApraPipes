...
class AffineTransform::Detail
{
	class MemoryStrategy
	{
		...
		void* fillMemory(frame_sp &frame);


	};
	class CPUMemoryStrategy
	{
		public:
			CPUMemoryStrategy()
			{
	...
	class DMAMemoryStrategy
	{
		public:
			...
			void* fillMemory(frame_sp &frame)
			{
				return static_cast<DMAFDWrapper *>(frame->data())->getCudaPtr();
			}
...
public:
	...
	{
		auto buffer = mMemoryStrategy->fillMemory(&inFrame);
...
public:
	...
	class CudaMemoryStrategy;
	boost::shared_ptr<CudaMemoryStrategy> mMemoryStrategy;
};
