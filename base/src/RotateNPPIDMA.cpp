...
public:
	...
	class DMAMemoryStrategy
	{

	};

	class CUDAmemoryStrategy
	{

	};
	
	Detail(RotateNPPIDMAProps &_props) : mProps(_props), shiftX(0), shiftY(0), mFrameType(FrameMetadata::GENERAL), mFrameLength(0)
	{
		nppStreamCtx.hStream = mProps.stream->getCudaStream();
	}
	...
	bool compute(void *buffer, void *outBuffer)
	{
		...
		{
			double si, co;
			si = mProps.scale * sin(mProps.angle * PI / 180);
			co = mProps.scale * cos(mProps.angle * PI / 180);
			double shx, shy;
			shx = (1 - co) * (srcSize[0].width / 2) + si * srcSize[0].height / 2;
			shy = (-si) * (srcSize[0].width / 2) + (1 - co) * srcSize[0].height / 2;
			double acoeff[2][3] = {{co, -si, shx + mProps.x}, {si, co, shy + mProps.y}};
...
bool RotateNPPIDMA::process(frame_container &frames)
{
    ...
    cudaMemsetAsync(static_cast<DMAFDWrapper *>(outFrame->data())->getCudaPtr(), 0, outFrame->size(), mDetail->mProps.stream->getCudaStream());
...
{
	RotateNPPIDMAProps props(mDetail->mProps.stream, 0);
