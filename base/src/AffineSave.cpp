		...
		if (mFrameType == FrameMetadata::RAW_IMAGE_PLANAR) // currently supported only for YUV444
		{
			...
			{   
				...
				if(!isOnce)
				{
					if(i == 2)
					{
						void* hostPtr2 = malloc(srcPitch[i] * srcSize[i].height * 3);
						cudaMemcpy(hostPtr2, (void*)src[i], srcPitch[i] * srcSize[i].height * 3, cudaMemcpyDeviceToHost);
						std::ofstream file3("ytvVchannel.raw", std::ios::out | std::ios::binary);
    					file3.write((const char*)hostPtr2, srcPitch[i] * srcSize[i].height * 3 );
    					file3.close();
						free(hostPtr2);
						isOnce = !isOnce;
					}
