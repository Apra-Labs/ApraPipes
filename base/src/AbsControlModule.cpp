...
bool AbsControlModule::handleCommand(Command::CommandType type,
                                     frame_sp &frame) {
  return true;
}

bool AbsControlModule::handlePropsChange(frame_sp &frame) { return true; }
...
bool AbsControlModule::term() { return Module::term(); }

bool AbsControlModule::process(frame_container &frames) { return true; }

std::string AbsControlModule::enrollModule(std::string pName, std::string role,
                                           boost::shared_ptr<Module> module) {
  std::string pipelineRole = mDetail->getPipelineRole(pName, role);
  if (moduleRoles.find(pipelineRole) != moduleRoles.end()) {
    std::string errMsg = "Enrollment Failed: This role <" + role +
                         "> already registered with the Module <" +
                         moduleRoles[pipelineRole]->getName() +
                         "> in PipeLine <" + pName + ">";
    LOG_ERROR << errMsg;
...
std::pair<bool, boost::shared_ptr<Module>>
AbsControlModule::getModuleofRole(std::string pName, std::string role) {
  std::string pipelineRole = mDetail->getPipelineRole(pName, role);
  if (moduleRoles.find(pipelineRole) == moduleRoles.end()) {
    return std::make_pair<bool, boost::shared_ptr<Module>>(false, nullptr);
  }
  std::pair<bool, boost::shared_ptr<Module>> res(true,
                                                 moduleRoles[pipelineRole]);
  return res;
}