...
{
	struct session_description sd = {(sdp_type)-1, NULL};
	int err = 0;
			     ...
			     const struct http_msg *msg, void *arg)
{
	...
	if (0 == pl_strcasecmp(&msg->met, "GET")) {
		...
		sess->conn_pending = (http_conn *)mem_ref(conn);
	}
...
void demo_close(void)
{
	...
	demo.httpssock = (http_sock *)mem_deref(demo.httpssock);
	demo.httpsock = (http_sock *)mem_deref(demo.httpsock);
	pc_config.ice_server = (stun_uri *)mem_deref(pc_config.ice_server);
}
