...
public:
    ...
    bool updateDetailStateOnPropsChange(RTSPClientSrcProps props)
    {
        LOG_DEBUG << "Updated Props==>>";
        LOG_DEBUG << "Updated RTSP Url " << props.rtspURL << " == Updated Connection Type TCP " << props.useTCP << " == URL TimeOut ==" << props.timeout << " ==";
        path = props.rtspURL;
        bUseTCP = props.useTCP;
        iUrlTimeout = props.timeout;
        int_ctx = std::make_pair(iUrlTimeout, start_time);
        int_cb = {interrupt_cb, &int_ctx};
        return true;
    }
...
{
    LOG_TRACE << "Destructor called for rtspclientsrc";
}
...
bool RTSPClientSrc::handlePropsChange(frame_sp &frame)
{
    RTSPClientSrcProps props;
    bool ret = Module::handlePropsChange(frame, props);
    mProps = props;
    mDetail->updateDetailStateOnPropsChange(mProps);
    mDetail->connect();
    return true;
}
