		...
		if (mFrameType == FrameMetadata::RAW_IMAGE_PLANAR) // currently supported only for YUV444
		{
			...
			for (auto i = 0; i < channels;  i++)
			{
				...
				if (saveFrame)
				{
					...
					{
						uptr = *src + (768*480); 
						void *hostPtr = malloc(srcPitch[0] * srcSize[0].height);
						cudaMemcpy(hostPtr, (void *)src[0], srcPitch[0] * srcSize[0].height, cudaMemcpyDeviceToHost);
						std::ofstream file3("Ychannel.raw", std::ios::out | std::ios::binary);
						file3.write((const char *)hostPtr, srcPitch[0] * srcSize[0].height);
						file3.close();
						free(hostPtr);
					}
					if (i == 1)
					{
						void *hostPtr2 = malloc(srcPitch[0] * srcSize[0].height);
						cudaMemcpy(hostPtr2, (void *)uptr, srcPitch[0] * srcSize[0].height, cudaMemcpyDeviceToHost);
						std::ofstream file3("Uchannel.raw", std::ios::out | std::ios::binary);
						file3.write((const char *)hostPtr2, srcPitch[0] * srcSize[0].height);
						file3.close();
						free(hostPtr2);
					}
					if (i == 2)
					{
						void *hostPtr = malloc(srcPitch[0] * srcSize[0].height);
						cudaMemcpy(hostPtr, (void *)src[2], srcPitch[0] * srcSize[0].height, cudaMemcpyDeviceToHost);
						std::ofstream file3("Vchannel.raw", std::ios::out | std::ios::binary);
						file3.write((const char *)hostPtr, srcPitch[0] * srcSize[0].height);
						file3.close();
						free(hostPtr);
					}
					void *hostPtr = malloc(srcPitch[0] * srcSize[0].height * 3);
					cudaMemcpy(hostPtr, buffer, srcPitch[0] * srcSize[0].height * 3, cudaMemcpyDeviceToHost);
					std::ofstream file3("YUVchannel.raw", std::ios::out | std::ios::binary);
					file3.write((const char *)hostPtr, srcPitch[0] * srcSize[0].height * 3);
					file3.close();
					free(hostPtr);
				}
			}
