...
{
public:
	Mp4readerDetailAbs(Mp4ReaderSourceProps& props,  const CallbackFunction &callback, std::function<frame_sp(size_t size, string& pinId)> _makeFrame,
	...
	bool initNewVideo()
	{
		...
		// 	if(mState.demux)
		// 	{
		// 		LOG_ERROR << "Nothing Needs to be done will just cstop reaing file";
		// 		mp4_demux_close(mState.demux);
		// 		mState.videotrack = -1;
		// 		mState.metatrack = -1;
		...
		if (mState.mFrameCounter == mState.mFramesInVideo)
		{
			...
			// 	mDetailCallbackFunction(); // Call the registered callback function
			// }
			// Video Completely Read
		}

		// no files left to read
		if (mState.end)
		{
			LOG_DEBUG << "<!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!NO FILES LEFT TO TO READ!--------------------------- ";
			// mState.mVideoPath = ""; /// zaki changes
		...
		if (ret < 0)
		{
			LOG_DEBUG << "mp4_demux_get_metadata_strings <" << -ret;
		}
			...
			for (auto i = 0; i < count; i++) {
				if ((keys[i]) && (values[i]))
				{
					...
					if (!strcmp(keys[i], "\251sts"))
					{
						...
						videoStartTime = mState.startTimeStamp;
					}
				}
			}
		}
		...
		if (mState.startTimeStamp)
		{
			LOG_DEBUG << "Start time Stamp greater than 0";
			openVideoStartingTS = mState.startTimeStamp;
		}
		else 
		{
			LOG_DEBUG << "Not Greater than 0";
			try
			{
				openVideoStartingTS = std::stoull(boostVideoTS);
			}
	...
	bool randomSeek(uint64_t& skipTS, uint64_t _seekEndTS)
	{
		...
		*/
		...
		if (!mProps.parseFS)
		{
			...
			if(!mState.demux)
			{
				LOG_ERROR << "DEMUX IS  A NULL POINTER";
			}
			...
			LOG_DEBUG << "seeked to frame" << seekedToFrame; //zaki
			LOG_DEBUG << "FRAME COUNTER" << mState.mFrameCounter;
		...
		if (ret < 0)
		{
			LOG_INFO << "Skip to ts <" << skipTS << "> failed. Please check skip dir <" << mProps.skipDir << ">";
			return false;
		}
		bool found = false;
		...
		if (skipMsecsInFile)
		{
			...
			if (returnCode < 0)
			{
				LOG_DEBUG << "Error while skipping to ts <" << skipTS << "> failed. File <" << skipVideoFile << "> @ time <" << skipMsecsInFile << "> ms errorCode <" << returnCode << ">";
				LOG_DEBUG << "Trying to seek to next available file...";
				auto nextFlag = mFSParser->getNextToVideoFileFlag();
				if (nextFlag < 0)
				{
					// reset the state of mux to before seek op
					LOG_DEBUG << "No next file found <" << nextFlag << ">" << "Resetting the reader state to before seek op";
					mState = tempState;
					LOG_DEBUG << "Switching back to video <" << mState.mVideoPath << ">";
	...
	CallbackFunction mDetailCallbackFunction;
protected:
...
struct DemuxAndParserState
	{
		...
		uint32_t mParsedFilesCount;
		uint32_t mVideoCounter;
		uint32_t mFrameCounter;
		uint32_t mFramesInVideo;
		std::vector<std::string> mParsedVideoFiles;
		std::string mVideoPath;
		bool randomSeekParseFlag;
		bool end;
	} mState;
...
{
public:
	Mp4readerDetailJpeg(Mp4ReaderSourceProps& props, const CallbackFunction &callback, std::function<frame_sp(size_t size, std::string& pinId)> _makeFrame,
		std::function<frame_sp(frame_sp& bigFrame, size_t& size, string& pinId)> _makeframe, std::function<void()> _sendEOS) : Mp4readerDetailAbs(props, callback, _makeFrame, _makeframe, _sendEOS)
	{}
...
{
public:
	Mp4readerDetailH264(Mp4ReaderSourceProps& props, const CallbackFunction &callback, std::function<frame_sp(size_t size, string& pinId)> _makeFrame,
		std::function<frame_sp(frame_sp& bigFrame, size_t& size, string& pinId)> _makeframe, std::function<void()> _sendEOS) : Mp4readerDetailAbs(props, callback, _makeFrame, _makeframe, _sendEOS)
	{}
	~Mp4readerDetailH264() { 
		// mp4_demux_close(mState.demux);
...
bool Mp4readerDetailH264::produceFrames(frame_container& frames)
{
	...
	{
		LOG_DEBUG << "DONE READING ALL THE FRAMES/////////////////////////////////////////////////////////////////////////";
	}
...
bool Mp4ReaderSource::handleCommand(Command::CommandType type, frame_sp& frame)
{
	if (type == Command::CommandType::Seek)
	{
		...
		LOG_DEBUG << "Seek Start Time " << mDetail->videoStartTime ;
		LOG_DEBUG << "Seek End Time " << seekCmd.seekEndTS ;
		return mDetail->randomSeek(mDetail->videoStartTime,seekCmd.seekEndTS);
	}
	else if (type == Command::CommandType::Mp4ReadCloseFile)
	{
		Mp4ReaderCloseFile closeFile;
		getCommand(closeFile, frame);
		return mDetail->closeFile();
	}
	else
	{
...
{
	LOG_DEBUG << "Got Random SEEK COMMAND =================================>>>>>>>>>>>>>>";
	Mp4SeekCommand cmd(seekStartTS, seekEndTS);
	return queueCommand(cmd);
}

bool Mp4ReaderSource::closeOpenFile()
{
