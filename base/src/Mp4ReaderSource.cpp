...
public:
	...
	{
		sentEOSSignal = false;
		LOG_INFO << "VIdeo Path is " << mState.mVideoPath;
	...
	{
		LOG_INFO << "===============Updating MState ==============" << videoPath;
	...
	{
		return mState.startTimeStampFromFile;
	}
	...
	void registerMp4FileTerminationCallback(const CallbackFunction &callback)
	{
		fileCompletionCallback = callback;
	}
	...
	*/
	bool openVideoSetPointer(std::string& filePath)
	{
		...
		for (auto i = 0; i < mState.ntracks; i++)
		{
			...
			if (mState.info.type == MP4_TRACK_TYPE_VIDEO && mState.videotrack == -1)
			{
				...
				mProps.fps = mProps.fps * playbackSpeed;
	...
	void readNextFrameInternal(frame_sp& imgFrame, frame_sp& metadetaFrame, size_t& imageFrameSize, size_t& metadataFrameSize, uint64_t& frameTSInMsecs, int32_t& mp4FIndex)
	{
		...
		if (waitFlag)
		{
			LOG_DEBUG << "readNextFrame: waitFlag <" << waitFlag << ">";
			std::chrono::time_point<std::chrono::system_clock> t = std::chrono::system_clock::now();
			auto dur = std::chrono::duration_cast<std::chrono::milliseconds>(t.time_since_epoch());
			uint64_t nowTS = dur.count();
...
public:
	...
	bool m_mediaBroke = false;
	bool mIsFrameStillAvailable;
	CallbackFunction callbackDurationFunction = NULL;
	CallbackFunction fileCompletionCallback;
};
...
bool Mp4ReaderSource::init()
{
	...
	mDetail->registerMp4FileTerminationCallback(callbackFunction);
	return mDetail->Init();
}
...
bool Mp4ReaderSource::closeCurrentOpenFile()
{
	Mp4ReaderCloseOpenFile cmd;
	return queueCommand(cmd);
}
...
int Mp4ReaderSource::getGOP()
{
	return mDetail->getGop();
}
...
uint64_t Mp4ReaderSource::getStartTimestamp()
{
	return mDetail->getStartTimestamp();
}

void Mp4ReaderSource::registerCallback(const CallbackFunction &callback)
{
