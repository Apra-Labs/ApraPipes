...
{
	struct session *sess = (session *)arg;
...
int session_new(struct list *sessl, struct session **sessp)
{
	...
	sess = (session *)mem_zalloc(sizeof(*sess), destructor);
	if (!sess)
		return ENOMEM;
			       ...
			       const struct http_msg *msg)
{
	...
	for (struct le *le = sessl->head; le; le = le->next) {
		...
		struct session *sess = (session *)le->data;
...
void session_close(struct session *sess, int err)
{
	...
	sess->pc = (peer_connection *)mem_deref(sess->pc);
