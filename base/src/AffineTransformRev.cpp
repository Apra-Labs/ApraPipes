...
class AffineTransformRev::Detail
{
public:
	Detail(AffineTransformRevProps &_props) : props(_props), shiftX(0), shiftY(0), mFrameType(FrameMetadata::GENERAL), mFrameLength(0)
	{
		nppStreamCtx.hStream = props.stream->getCudaStream();
		rowSize[4] = 0;
		height[4] = 0;
	}
	int setInterPolation(AffineTransformRevProps::Interpolation eInterpolation)
	{
		switch (props.eInterpolation)
		{
		case AffineTransformRevProps::NN:
			return NppiInterpolationMode::NPPI_INTER_NN;
		case AffineTransformRevProps::LINEAR:
			return NppiInterpolationMode::NPPI_INTER_LINEAR;
		case AffineTransformRevProps::CUBIC:
			return NppiInterpolationMode::NPPI_INTER_CUBIC;
		case AffineTransformRevProps::UNDEFINED:
			return NppiInterpolationMode::NPPI_INTER_UNDEFINED; // not supported
		case AffineTransformRevProps::CUBIC2P_BSPLINE:
			return NppiInterpolationMode::NPPI_INTER_CUBIC2P_BSPLINE; // not supported
		case AffineTransformRevProps::CUBIC2P_CATMULLROM:
			return NppiInterpolationMode::NPPI_INTER_CUBIC2P_CATMULLROM;
		case AffineTransformRevProps::CUBIC2P_B05C03:
			return NppiInterpolationMode::NPPI_INTER_CUBIC2P_B05C03; // not supported
		case AffineTransformRevProps::SUPER:
			return NppiInterpolationMode::NPPI_INTER_SUPER; // not supported
		case AffineTransformRevProps::LANCZOS:
			return NppiInterpolationMode::NPPI_INTER_LANCZOS; // not supported
		case AffineTransformRevProps::LANCZOS3_ADVANCED:
	...
	void setMetadata(framemetadata_sp &metadata)
	{
		...
		setAffineTransformRevMatrix();
		DMAAllocator::setMetadata(mOutputMetadata, dstSize[0].width, dstSize[0].height, ImageMetadata::ImageType::RGBA);
		
	}
	bool isOnce = false;

	bool setAffineTransformRevMatrix()
	{
	...
	bool compute(void *buffer, void *outBuffer)
	{
		...
		if (mFrameType == FrameMetadata::RAW_IMAGE)
		{
			...
			else if (channels == 4)
			{
				...
				{
					LOG_DEBUG << "AffineTransformRev failed<" << status << ">";
				}
			}
		}
		if (status != NPP_SUCCESS)
		{
			LOG_ERROR << "AffineTransformRev failed<" << status << ">";
			throw AIPException(AIP_FATAL, "Failed to tranform the image");
		}
		return true;
	}

	void setProps(AffineTransformRevProps &mprops)
	{
		props = mprops;
		setAffineTransformRevMatrix();
	}
	...
	bool setMetadataHelper(framemetadata_sp &input, framemetadata_sp &output)
	{
		...
		else if (mFrameType == FrameMetadata::RAW_IMAGE_PLANAR)
		{
			...
			for (auto i = 0; i < channels; i++)
			{
				...
				srcPitch[i] = static_cast<int>(inputRawMetadata->getStep(i));
				// srcNextPtrOffset[i] = inputRawMetadata->getNextPtrOffset(i);
				srcNextPtrOffset[i] = i * 262144; // works for 400x400
				rowSize[i] = inputRawMetadata->getRowSize(i);
				height[i] = inputRawMetadata->getHeight(i);
				...
				dstPitch[i] = static_cast<int>(outputRawMetadata->getStep(i));
				// dstNextPtrOffset[i] = outputRawMetadata->getNextPtrOffset(i);
				dstNextPtrOffset[i] = i * 262144; // works for 400x400
				// dstNextPtrOffset[i] = i * 917504;
			}
		}
		return true;
	}
...
AffineTransformRev::AffineTransformRev(AffineTransformRevProps props) : Module(TRANSFORM, "AffineTransformRev", props)
{
	mDetail.reset(new Detail(props));
}

AffineTransformRev::~AffineTransformRev() {}

bool AffineTransformRev::validateInputPins()
{
...
bool AffineTransformRev::validateOutputPins()
{
...
void AffineTransformRev::addInputPin(framemetadata_sp &metadata, string &pinId)
{
...
bool AffineTransformRev::term()
{
	mDetail.reset();
	return Module::term();
}

bool AffineTransformRev::process(frame_container &frames)
{
...
bool AffineTransformRev::processSOS(frame_sp &frame)
{
...
bool AffineTransformRev::shouldTriggerSOS()
{
	return mDetail->mFrameLength == 0;
}

bool AffineTransformRev::processEOS(string &pinId)
{
	mDetail->mFrameLength = 0;
	return true;
}

void AffineTransformRev::setProps(AffineTransformRevProps &props)
{
	Module::addPropsToQueue(props, true);
}

AffineTransformRevProps AffineTransformRev::getProps()
{
	fillProps(mDetail->props);
	return mDetail->props;
}

bool AffineTransformRev::handlePropsChange(frame_sp &frame)
{

	AffineTransformRevProps props(mDetail->props.stream, 0);
