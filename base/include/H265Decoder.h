...
class H265DecoderProps : public ModuleProps
{
public:
	H265DecoderProps(uint _lowerWaterMark = 300, uint _upperWaterMark = 350)
	{
...
class H265Decoder : public Module
{
public:
	H265Decoder(H265DecoderProps _props);
	virtual ~H265Decoder();
	bool init();
	bool term();
...
private:
	...
	void saveVpsSpsPps(frame_sp frame);
	void* prependVpsSpsPps(frame_sp& iFrame, size_t& vpsSpsPpsFrameSize);
	void dropFarthestFromCurrentTs(uint64_t ts);
	frame_sp mHeaderFrame;
	boost::asio::const_buffer vpsBuffer;
