...
public:
  ArchiveSpaceManagerProps(uint64_t _lowerWaterMark, uint64_t _upperWaterMark,
                           string _pathToWatch, int _samplingFreq) {
    ...
    // auto totalSpace = std::space(pathToWatch);
    // if ((lowerWaterMark > upperWaterMark) ||
    //     (upperWaterMark > totalSpace.capacity)) {
    //   LOG_ERROR << "Please enter correct properties!";
    //   std::string errorMsg =
    //       "Incorrect properties set for Archive Manager. TotalDiskCapacity <"
    //       + std::to_string(totalSpace.capacity) + ">lowerWaterMark<" +
    //       std::to_string(lowerWaterMark) + "> UpperWaterMark<" +
    //       std::to_string(upperWaterMark) + ">";
...
size_t getSerializeSize() {
  return ModuleProps::getSerializeSize() + sizeof(lowerWaterMark) +
         sizeof(upperWaterMark) + sizeof(pathToWatch) + sizeof(samplingFreq);
}
