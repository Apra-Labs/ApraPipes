...
// Include auto-generated list of all Module subclasses
// Generated by cmake/ScanModuleClasses.cmake at configure time
#include "module_subclasses.inc"
...
BOOST_AUTO_TEST_CASE(DetectUnregisteredModules_InfoTest) {
    ...
    //
    // D2 Phase 3: Uses auto-generated list from cmake/ScanModuleClasses.cmake
    ...
    // Use auto-generated list of all Module subclasses
    // This list is generated at cmake configure time from header files
    const auto& allModules = ALL_MODULE_SUBCLASSES;
    size_t totalModules = allModules.size();

    // Find unregistered modules
    std::vector<std::string> unregistered;
    std::vector<std::string> registeredList;
    for (const auto& module : allModules) {
        if (registeredSet.find(module) == registeredSet.end()) {
            unregistered.push_back(module);
        } else {
            registeredList.push_back(module);
        }
    }

    // Calculate coverage
    size_t registeredCount = registeredList.size();
    double coveragePercent = (totalModules > 0)
        ? (100.0 * registeredCount / totalModules)
        : 0.0;

    // Print coverage header
    BOOST_TEST_MESSAGE("");
    BOOST_TEST_MESSAGE("╔═══════════════════════════════════════════════════════════════╗");
    BOOST_TEST_MESSAGE("║             MODULE REGISTRATION COVERAGE REPORT               ║");
    BOOST_TEST_MESSAGE("╠═══════════════════════════════════════════════════════════════╣");
    BOOST_TEST_MESSAGE("║  Total Modules Found:     " << std::setw(3) << totalModules << "                                 ║");
    BOOST_TEST_MESSAGE("║  Registered:              " << std::setw(3) << registeredCount << "                                 ║");
    BOOST_TEST_MESSAGE("║  Unregistered:            " << std::setw(3) << unregistered.size() << "                                 ║");
    BOOST_TEST_MESSAGE("║  Coverage:                " << std::fixed << std::setprecision(1) << std::setw(5) << coveragePercent << "%                              ║");
    BOOST_TEST_MESSAGE("╚═══════════════════════════════════════════════════════════════╝");
    BOOST_TEST_MESSAGE("");

    // Show registered modules
    if (!registeredList.empty()) {
        BOOST_TEST_MESSAGE("✅ Registered Modules (" << registeredList.size() << "):");
        for (const auto& name : registeredList) {
            BOOST_TEST_MESSAGE("   ✓ " << name);
        }
        BOOST_TEST_MESSAGE("");
    }

    // Show unregistered modules
    if (!unregistered.empty()) {
        BOOST_TEST_MESSAGE("⚠️  Unregistered Modules (" << unregistered.size() << "):");
        for (const auto& name : unregistered) {
            BOOST_TEST_MESSAGE("   ✗ " << name);
        }
        BOOST_TEST_MESSAGE("");
        BOOST_TEST_MESSAGE("Add registrations to: base/src/declarative/ModuleRegistrations.cpp");
        BOOST_TEST_MESSAGE("Re-scan with: cmake --build . --target scan-modules");
    }
