...
BOOST_AUTO_TEST_CASE(basic, *boost::unit_test::disabled())
{
	...
	for (auto i = 0; i < n_cams; i++)
	{
		...
		auto source = std::shared_ptr<Module>(new NvArgusCamera(sourceProps));
		...
		if (n_shouldrender > 0)
		{
			...
			auto renderer = std::shared_ptr<Module>(new EglRenderer(renderProps));
...
BOOST_AUTO_TEST_CASE(vcam_nv12, *boost::unit_test::disabled())
{
	...
	// auto fileWriter = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/nvargus/nv12_????.raw")));
...
BOOST_AUTO_TEST_CASE(vcam_yuv420, *boost::unit_test::disabled())
{
	...
	auto transform = std::shared_ptr<Module>(new NvTransform(NvTransformProps(ImageMetadata::YUV420)));
	source->setNext(transform);

	auto copy = std::shared_ptr<Module>(new DMAFDToHostCopy);
	...
	// auto fileWriter = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/nvargus/yuv420_????.raw")));
	// copy->setNext(fileWriter);
	// auto fileWriter2 = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/nvargus/nv12_????.raw")));
...
BOOST_AUTO_TEST_CASE(vcam, *boost::unit_test::disabled())
{
	...
	auto transform = std::shared_ptr<Module>(new NvTransform(NvTransformProps(ImageMetadata::BGRA)));
	source->setNext(transform);
	...
	// auto fileWriter = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/nvargus/bgra_????.raw")));
	// copy->setNext(fileWriter);
	// auto fileWriter2 = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/nvargus/nv12_????.raw")));
	// copySource->setNext(fileWriter2);
...
BOOST_AUTO_TEST_CASE(encoder, *boost::unit_test::disabled())
{
	...
	auto fileWriter = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/ArgusCamera/frame.h264", true)));
...
BOOST_AUTO_TEST_CASE(encoderrtsppush, *boost::unit_test::disabled())
{
	...
	// auto fileWriter = std::shared_ptr<Module>(new FileWriterModule(FileWriterModuleProps("./data/testOutput/ArgusCamera_1280x720.h264", true)));
	// encoder->setNext(fileWriter);

	RTSPPusherProps rtspPusherProps("rtsp://10.102.10.129:5544", "aprapipes_h264");
	auto rtspPusher = std::shared_ptr<Module>(new RTSPPusher(rtspPusherProps));
