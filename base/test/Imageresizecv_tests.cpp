...
BOOST_AUTO_TEST_CASE(mono1_1920x960_shrink)
{
	...
	auto resize = std::shared_ptr<ImageResizeCV>(new ImageResizeCV(ImageResizeCVProps(200, 200)));
...
BOOST_AUTO_TEST_CASE(mono1_1920x960_expand)
{
	...
	auto resize = std::shared_ptr<ImageResizeCV>(new ImageResizeCV(ImageResizeCVProps(2000, 2000)));
	fileReader->setNext(resize);
...
BOOST_AUTO_TEST_CASE(perf, *boost::unit_test::disabled())
{
	...
	auto m2 = std::shared_ptr<Module>(new ImageResizeCV(ImageResizeCVProps(width >> 1, height >> 1)));
