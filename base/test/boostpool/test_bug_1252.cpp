...
void test_alignment(T)
{
   ...
   ApraPool<boost::default_user_allocator_new_delete> p(sizeof(T));
   unsigned limit = 100000;
   for(unsigned i = 0; i < limit; ++i)
   {
      void* ptr = (p.ordered_malloc)(1);
...
BOOST_AUTO_TEST_CASE(test_pool_bug_1252)
{
   ApraPool<limited_allocator_new_delete> po(1501);
   void* p = (po.ordered_malloc)(1);
   BOOST_TEST(p != 0);
