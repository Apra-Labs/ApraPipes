...
BOOST_AUTO_TEST_CASE(stop, * boost::unit_test::disabled())
{
...

	{
		...
		auto t2 = std::thread(ref(*(m2.get())));
		auto t1 = std::thread(ref(*(m1.get()))); // running m1 again - stop was called before so thread would have exited
		std::this_thread::sleep_for(std::chrono::milliseconds(100));  // giving time to call step 
		m1->stop();
		m1->step();
		std::this_thread::sleep_for(std::chrono::milliseconds(100));  // giving time to call step 
