...
* utf::precondition(if_h264_encoder_supported()))
{
	...
	p->wait_for_all();
	p.reset();
	std::string fileComparePath = "./data/H264EncoderNvCodecTests/Raw_YUV420_640x360_to_160x90_Compare.h264";
	uint8_t* frameData;
	uint frameSize;
...
* utf::precondition(if_h264_encoder_supported()))
{
	...
	Test_Utils::saveOrCompare(fileComparePath.c_str(), (const unsigned char*)frameData, (size_t)frameSize, 0);	Test_Utils::saveOrCompare(outFile[0], 0);
}
